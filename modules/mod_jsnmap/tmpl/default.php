...
?>
  ...
  var marker<?php echo $user->id.'_'.$map_alias.'_'.$module->id; ?> = new google.maps.Marker({
      ...
      optimized:false,
      icon: {
      url: '<?php echo JURI::root(true).'/'.$user->avatar_mini; ?>',
      scaledSize: size
    }<?php endif; ?>
  });
  google.maps.event.addListener(marker<?php echo $user->id.'_'.$map_alias.'_'.$module->id; ?>, 'click', function() {
    ...
    <?php if($jsnConfig->get('avatarletters',0)) : ?>LetterAvatar.transform();<?php endif; ?>
  });
...
<?php if($tot==1) : ?>google.maps.event.addListenerOnce(jsnmap<?php echo $module->id; ?>, 'bounds_changed', function(event) {
