...
class JsnGmapFieldHelper
{
	...
	public static function getXml($item)
	{
		...
		if(!in_array($item->alias,$loaded)/* && JFactory::getApplication()->isSite()*/)
		{
			...
			jQuery(document).ready(function(){
				...
				'.($item->params->get('gmap_route',0) ? 'jQuery(".'.$item->alias.'Value").append("<br /><a class=\"btn btn-info btn-small btn-sm\" target=\"_blank\" href=\"https://www.google.com/maps/dir/Current+Location/"+lat+","+lng+"\"><i class=\"jsn-icon jsn-icon-globe\"></i> '.JText::_('COM_JSN_GETDIRECTIONS').'</a>");' : '').
			'});
			';
	...
	public static function storeData($field, $data, &$storeData)
	{
		...
		if(isset($data[$alias.'_lat'])) $storeData[$alias.'_lat']=$data[$alias.'_lat'];
		if(isset($data[$alias.'_lng'])) $storeData[$alias.'_lng']=$data[$alias.'_lng'];
	}
