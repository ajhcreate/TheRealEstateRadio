...
class JsnSelectlistFieldHelper
{
	...
	public static function getSearchInput($field)
	{
		...
		{	
			...
			{
				$script = JsnSelectlistFieldHelper::getAjaxScript($dbOptFilterValue,$field->alias);
	...
	public static function operations()
	{
		...
		if(!$params->get('select_multiple',0)) $options[]=JHtml::_('select.option', '', JText::_('COM_JSN_NOSELECTION'));
		foreach($optTxt as $opt)
		{
			$opt=trim($opt);
			if($opt!=''){
	...
	public static function getAjaxScript($parentField,$thisField,$prefix='jform_')
	{	$rand=rand(0,10000);
		$script='<script>
		jQuery(window).load(function(){
			...
			if(jQuery("#'.$prefix.$parentField.'").val()!="")
			{
				...
				jQuery.ajax({
				    url:"index.php?option=com_jsn&view=opField&type=selectlist&field='.$thisField.'&value="+jQuery("#'.$prefix.$parentField.'").val()+"&format=raw",
				    type:"GET",
				    data: "",
				    dataType: "json",
			...
			jQuery("#'.$prefix.$parentField.'").change(function(){
				...
				jQuery.ajax({
				    url:"index.php?option=com_jsn&view=opField&type=selectlist&field='.$thisField.'&value="+jQuery(this).val()+"&format=raw",
				    type:"GET",
				    data: "",
				    dataType: "json",
				    success: function( json ) {
