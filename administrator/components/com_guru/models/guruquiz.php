...
class guruAdminModelguruQuiz extends JModelLegacy {
	...
	function addquestion ($qtext,$quizid,$question_type,$media_ids,$points,$true_false_ch, $question_id, $from_save_or_not, $ans_content) {
		...
		if($question_type == 'true_false'){
			...
			if(intval($question_id) == 0){
				$sql = "INSERT INTO #__guru_question_answers(id, answer_content_text, media_ids, correct_answer, question_id) VALUES ('', '".addslashes(JText::_("GURU_QUESTION_OPTION_TRUE"))."', '' , '".addslashes($correct_ans_true)."',".intval($id_question["0"]).");";
				$db->setQuery($sql);
				if (!$db->query() ){
					$this->setError($db->getErrorMsg());
					return false;
				...
				$sql = "INSERT INTO #__guru_question_answers(id, answer_content_text, media_ids, correct_answer, question_id) VALUES ('', '".addslashes(JText::_("GURU_QUESTION_OPTION_FALSE"))."', '' , '".addslashes($correct_ans_false)."', ".intval($id_question["0"]).");";
				$db->setQuery($sql);
				if (!$db->query() ){
					$this->setError($db->getErrorMsg());
					return false;
