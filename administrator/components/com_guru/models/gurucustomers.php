...
class guruAdminModelguruCustomers extends JModelLegacy{
	...
	function resetCourses(){
		...
		$sql = "select question_id from #__guru_quiz_taken_v3 where pid=".intval($course_id)." and user_id=".intval($user_id);
		$db->setQuery($sql);
		...
		$sql = "delete from #__guru_quiz_question_taken_v3 where user_id=".intval($user_id);
		$db->setQuery($sql);
		if(!$db->query()){
			return "0";
		}
		
		$sql = "delete from #__guru_quiz_taken_v3 where pid=".intval($course_id)." and user_id=".intval($user_id);
		$db->setQuery($sql);
		if(!$db->query()){
			return "0";
		}
