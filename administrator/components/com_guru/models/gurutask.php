...
<embed type="application/x-mplayer2" pluginspage="http://www.microsoft.com/Windows/MediaPlayer/" src="'.JURI::root().$configs->videoin."/".$the_media->local.'" name="MediaPlayer1" width="'.$vwidth.'" height="'.$vheight.'" autostart="1" showcontrols="1" volume="10">
	...
	function store(){
		...
		if(intval($data_post['jumpbutton1'])!=0){
			$sql1="INSERT INTO #__guru_mediarel (type,type_id,media_id,mainmedia,text_no) VALUES ('jump','".$data['id']."','".intval($data_post['jumpbutton1'])."','0','0')";
			$db->setQuery($sql1);
			$db->query($sql1);
		}
		if(intval($data_post['jumpbutton2'])!=0){	
			$sql="INSERT INTO #__guru_mediarel (type,type_id,media_id,mainmedia,text_no) VALUES ('jump','".$data['id']."','".intval($data_post['jumpbutton2'])."','0','0')";
			$db->setQuery($sql);
			$db->query($sql);
		}
		
		if(intval($data_post['jumpbutton3'])!=0){
			$sql="INSERT INTO #__guru_mediarel (type,type_id,media_id,mainmedia,text_no) VALUES ('jump','".$data['id']."','".intval($data_post['jumpbutton3'])."','0','0')";
			$db->setQuery($sql);
			$db->query($sql);
		}

		if(intval($data_post['jumpbutton4'])!=0){
			$sql="INSERT INTO #__guru_mediarel (type,type_id,media_id,mainmedia,text_no) VALUES ('jump','".$data['id']."','".intval($data_post['jumpbutton4'])."','0','0')";
			$db->setQuery($sql);
			$db->query($sql);
		}
