			...
			foreach ($thefiles as $files){
				if($files != ""){
					...
					if (intval($files)>0 && !in_array($files,$id_tmp_med_files_2_remove)) {
						...
						$db = JFactory::getDbo();
						$sql = "insert into #__guru_mediarel(`id`, `type`, `type_id`, `media_id`, `mainmedia`, `access`, `order`) values ('', 'pmed', '".trim($progid)."', '".trim($files)."', '0', '".trim($data[$access])."', '".trim($order)."')";
						$db->setQuery($sql);
						$db->query();
					}
				}
		...
		foreach($media_rel_object_list as $media_rel_object){
			...
			if($media_rel_object->type == "dtask"){
				...
				if(isset($old_lesson) && count($old_lesson) > 0){
					$sql = "INSERT INTO #__guru_task (name, alias, category, difficultylevel, points, image, published, startpublish, endpublish, metatitle, metakwd, metadesc, time, ordering, step_access, final_lesson, forum_kunena_generatedt, groups_access, duration) VALUES ('".addslashes(trim($old_lesson["0"]["name"]))."', '".addslashes(trim($old_lesson["0"]["alias"]))."', ".intval($old_lesson["0"]["category"]).", '".trim($old_lesson["0"]["difficultylevel"])."', ".intval($old_lesson["0"]["points"]).", '".trim($old_lesson["0"]["image"])."', '".$old_lesson["0"]["published"]."', '".$old_lesson["0"]["startpublish"]."', '".$old_lesson["0"]["endpublish"]."', '".addslashes(trim($old_lesson["0"]["metatitle"]))."', '".addslashes(trim($old_lesson["0"]["metakwd"]))."', '".addslashes(trim($old_lesson["0"]["metadesc"]))."', ".$old_lesson["0"]["time"].", ".$old_lesson["0"]["ordering"].", ".$old_lesson["0"]["step_access"].", '".$old_lesson["0"]["final_lesson"]."', '".$old_lesson["0"]["forum_kunena_generatedt"]."', '".$old_lesson["0"]["groups_access"]."', '".$old_lesson["0"]["duration"]."')";
					$db->setQuery($sql);
					if($db->query()){
