...
class guruAdminModelguruinstall extends JModelLegacy {
	...
	function startDatabaseInstall(){
		...
		$sql = "CREATE TABLE IF NOT EXISTS `#__guru_config` (
				  ...
				  `mailchimp_teacher_api` varchar(255) NOT NULL,
				  `mailchimp_teacher_list_id` varchar(255) NOT NULL,
				  `mailchimp_teacher_auto` int(3) NOT NULL,
				  `mailchimp_student_api` varchar(255) NOT NULL,
				  `mailchimp_student_list_id` varchar(255) NOT NULL,
				  `mailchimp_student_auto` int(3) NOT NULL,
				  `seo` text NOT NULL,
				  `captcha` int(3) NOT NULL DEFAULT '1',
				  `auto_approve` int(3) NOT NULL DEFAULT '1',
				  `youtube_key` varchar(255) NOT NULL,
				  `course_certificate` int(3) NOT NULL,
				  `course_exercises` int(3) NOT NULL,
				  `rtl` int(3) NOT NULL,
				  PRIMARY KEY (`id`)
				) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;";
		$db->setQuery($sql);
		...
		$sql = "CREATE TABLE IF NOT EXISTS `#__guru_quiz` (
				  ...
				  `pending_message` LONGTEXT NOT NULL DEFAULT '',
				  PRIMARY KEY (`id`)
				) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;";
		$db->setQuery($sql);
		$db->query();
		...
		if(isset($result)){
			...
			if(!in_array("sequential_courses", $result)){
				$sql = "ALTER TABLE `#__guru_customer` ADD `sequential_courses` TEXT DEFAULT ''";
				$db->setQuery($sql);
				$db->query();
			}
		}
		...
		if(isset($result)){
			...
			if(!in_array("groups_access", $result)){
				$sql = "ALTER TABLE `#__guru_program` ADD `groups_access` TEXT NULL";
				$db->setQuery($sql);
				$db->query();
			}
		...
		if(isset($result)){
			...
			if(!in_array("mailchimp_teacher_api", $result)){
				$sql = "ALTER TABLE `#__guru_config` ADD `mailchimp_teacher_api` varchar(255) NOT NULL DEFAULT ''";
				$db->setQuery($sql);
				$db->query();
			}
			...
			if(!in_array("seo", $result)){
				$sql = "ALTER TABLE `#__guru_config` ADD `seo` TEXT NOT NULL";
				$db->setQuery($sql);
				$db->query();
			}
			...
			if(!in_array("course_certificate", $result)){
				$sql = "ALTER TABLE `#__guru_config` ADD `course_certificate` INT(3) NOT NULL";
				$db->setQuery($sql);
				$db->query();
			}
			if(!in_array("course_exercises", $result)){
				$sql = "ALTER TABLE `#__guru_config` ADD `course_exercises` INT(3) NOT NULL";
				$db->setQuery($sql);
				$db->query();
			}
			if(!in_array("rtl", $result)){
				$sql = "ALTER TABLE `#__guru_config` ADD `rtl` INT(3) NOT NULL";
				$db->setQuery($sql);
				$db->query();
			}
		}
		...
		if(isset($result)){
			...
			if(!in_array("pending_message", $result)){
				$sql = "ALTER TABLE `#__guru_quiz` ADD `pending_message` LONGTEXT NOT NULL DEFAULT ''";
				$db->setQuery($sql);
				$db->query();
			}
 		}
		...
		if(isset($result)){
			if(!in_array("groups_access", $result)){
				$sql = "ALTER TABLE `#__guru_task` ADD `groups_access` text NOT NULL";
				$db->setQuery($sql);
				$db->query();
			}
			...
			if(!in_array("description", $result)){
				$sql = "ALTER TABLE `#__guru_task` ADD `description` LONGTEXT DEFAULT ''";
				$db->setQuery($sql);
				$db->query();
			}
		}
	...
	function startInstallPlugins(){
		...
		if(!JFile::copy($source, $destination, '', true)){
			echo "Can't copy ".$source;
		}
