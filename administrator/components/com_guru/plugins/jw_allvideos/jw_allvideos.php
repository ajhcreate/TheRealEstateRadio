...
class plgContentJw_allvideos extends JPlugin {
  ...
  // JoomlaWorks reference parameters
    ...
    if(JPluginHelper::isEnabled('content',$this->plg_name)==false) return;
		...
		if(JFactory::getApplication()->input->get('format')=='html' || JFactory::getApplication()->input->get('format')==''){
		...
		foreach ($tagReplace as $plg_tag => $value) {
			...
			if(preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER)) {
				...
				foreach ($matches[0] as $key => $match) {
					...
					// Poster frame
					$posterFramePath = $sitePath.DIRECTORY_SEPARATOR.str_replace('/', DIRECTORY_SEPARATOR,$vfolder);
					if(JFile::exists($posterFramePath.DIRECTORY_SEPARATOR.$tagsource.'.jpg')){
						$output->posterFrame = $siteUrl.'/'.$vfolder.'/'.$tagsource.'.jpg';
					} elseif(JFile::exists($posterFramePath.DIRECTORY_SEPARATOR.$tagsource.'.png')){
						$output->posterFrame = $siteUrl.'/'.$vfolder.'/'.$tagsource.'.png';
					} elseif(JFile::exists($posterFramePath.DIRECTORY_SEPARATOR.$tagsource.'.gif')){
