...
function addmedia (idu, name, asoc_file, description) {
	...
	<div style="float: left; font-weight:bold"><?php echo JText::_("GURU_CLICK_TO_QUIZ"); ?></div>
		<br /><br />
	<div>
<form name="adminForm2" action="index.php?option=com_guru&controller=guruTasks&task=addquiz&med=<?php echo $data_get['med'];?>&tmpl=component&cid[]=<?php echo $data_get['cid'][0];?><?php if(isset($data_get['quiz'])){echo "&quiz=".$data_get['quiz'];}?><?php if(isset($_REQUEST['type'])){echo "&type=".$_REQUEST['type'];}?>" method="post">
			...
			<td class="g_top_filters">
            	<?php
					...
					if(isset($data_post['search_quiz'])&&($data_post['search_quiz']!='')){
						$search_quiz_tskmed = $data_post['search_quiz'];
					}
				?>
                
				<input type="text" name="search_quiz" value="<?php echo $search_quiz_tskmed; ?>" />
...
<?php 
		...
		if ($n>0) { 
			...
			$asoc_file = "-";
			$link = "addmedia('".$id."', '".addslashes($file->name)."', '".addslashes($asoc_file)."', '' ); ";
	...
	<form name="adminForm" id="adminForm" action="index.php?option=com_guru&controller=guruTasks&task=addquiz&med=<?php echo $data_get['med'];?>&tmpl=component&cid[]=<?php echo $data_get['cid'][0];?>" method="post">
		<table>
        	<tr>
        		<td colspan="6">
