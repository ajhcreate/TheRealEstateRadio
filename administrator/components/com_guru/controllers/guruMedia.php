						...
						// for adding sound ?>						
							...
							new Ajax.Request(url, {
								...
								onSuccess: function(transport) {
									...
									to_be_replaced.innerHTML = '&nbsp;';
									if(replace_m!=99){
										if((transport.responseText.trim().match(/(.*?).pdf(.*?)/))&&(!transport.responseText.trim().match(/(.*?).iframe(.*?)/))){
											to_be_replaced.innerHTML += transport.responseText.trim()+'<p /><div  style="text-align:center"><i>' + description + '</i></div>'; 
										}
										else{
											to_be_replaced.innerHTML += transport.responseText.trim()+'<br /><div style="text-align:center"><i>'+ name +'</i></div><br /><div  style="text-align:center"><i>' + description + '</i></div>';
										}
									}
									else{
										to_be_replaced.innerHTML += transport.responseText.trim();
									...
									if ((transport.responseText.trim().match(/(.*?).pdf(.*?)/))&&(!transport.responseText.trim().match(/(.*?).iframe(.*?)/))) {
										var qwe='&nbsp;'+transport.responseText.trim()+'<p /><div  style="text-align:center"><i>' + description + '</i></div>';
									} 
									else {
										var qwe='&nbsp;'+transport.responseText.trim()+'<br /><div style="text-align:center"><i>'+ name +'</i></div><div  style="text-align:center"><i>' + description + '</i></div>';
									}
