...
class guruAdminControllerguruSubplan extends guruAdminController {
    ...
    function save(){
        ...
        if($this->_model->store()){
            ...
            $app->redirect('index.php?option=com_guru&controller=guruSubplan', $msg);
        } 
		else{
    ...
    function apply(){
        ...
        if($id = $this->_model->store()){
            $msg = JText::_('GURU_MODIF_OK');
            $app->redirect('index.php?option=com_guru&controller=guruSubplan&task=edit&cid[]='.$id, $msg);
        }
		else{
            $msg = JText::_('GURU_ERROR');
            $app->redirect('index.php?option=com_guru&controller=guruSubplan', $msg);
    ...
    function publish(){
		...
		elseif($return === FALSE){
			...
			$app->redirect('index.php?option=com_guru&controller=guruSubplan', $return);
		}
        $this->display();
    }
