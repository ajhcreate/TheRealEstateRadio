 ...
 * @subpackage	models.fields.joomfuserepeatable
 *
...
class JFormFieldJoomfuserepeatable extends JFormField {
    protected $type = 'joomfuserepeatable';

    protected $childElements = array();
    protected $numRows = 0;

    //Array holding a single row of child elements with no value that are hidden and used from the 'add row' button
    protected $templateChildElements = array();
    ...
    public function setup($element, $value, $group = null){
        ...
        JFactory::getDocument()->addScript(JURI::root().'/administrator/components/com_joomfuse/models/fields/joomfuserepeatable.js');
        JFactory::getDocument()->addStyleSheet(JURI::root().'/administrator/components/com_joomfuse/models/fields/joomfuserepeatable.css');
        ...
        //Setup the template rows
        foreach($element->field AS $child_element){
            $this->templateChildElements[(string)$child_element['name']] = $this->setupChildelement($child_element, array(), $group, $element, 0);
        }

        //Set the width of each input field column according to the number of rows
        $container_width = 100/count($element->field)-5;
        //JFactory::getDocument()->addStyleDeclaration('.joomfuse_entry_container {width: '. $container_width .'%;}');
    ...
    protected function getInput(){
        ...
        //Table footer: The template row
        $returnString .= '<tfoot><tr class="joomfuserepeatable_template_row" data-forrepeatable="'.$this->name.'" style="display:none">';
        //Append the move icon
        $returnString .= '<td>'.$this->getMoveIconHTML().'</td>';
        foreach($this->templateChildElements AS $templateElement){
            $returnString .= '<td>'.$templateElement->getInput().'</td>';
        }
        $returnString .= '<td>'.$this->getRemoveIconHTML().'</td>';
        $returnString .= '</tr></tfoot>';
    ...
    /**
     * Returns the 'remove' icon used for the jQuery.sortable.
     * More of a normalizations function than a helper one
     * @return string	The HTML for the remove icon
     */
    protected function getRemoveIconHTML(){
        return '<a class="joomfuse_remove" href="#" onClick="joomfuserepeatable_remove_row(this)"></a>';
    }
