	...
	<?php
		...
		if ($this->params->get('template.show_categories', true) && ($this->category->childrenHaveItems() || $this->params->get('config.show_empty_categories', false))) {
			...
			foreach ($this->selected_categories as $category) {
				...
				if ($i % $this->params->get('template.categories_cols') == 0) echo ($i > 0 ? '</div><div class="uk-grid" uk-grid>' : '<div class="uk-grid" uk-grid>');
				echo '<div class="uk-width-1-'.$this->params->get('template.categories_cols').'@m">'.$this->partial('frontpage_category', compact('category')).'</div>';
				$i++;
			}
