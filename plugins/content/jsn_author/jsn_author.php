	...
	public function onContentBeforeDisplay( $context, &$article ) {
		...
		if(isset($article->created_by) && isset($article->author) && $article->created_by && !is_object($article->author)){
			...
			if($config->get('profileACL',2)==0){
				if($config->get('avatar',1) && $show_avatar) $article->author=$user->getFormatName().' <img src="'.$user->avatar_mini.'" class="avatar author_avatar"/> ';
				else $article->author=$user->getFormatName();
			}
			else
			{
				if($config->get('avatar',1) && $show_avatar) $article->author='<a href="'.$user->getLink().'">'.$user->getFormatName().' <img src="'.$user->avatar_mini.'" class="avatar author_avatar"/> </a>';
				else $article->author='<a href="'.$user->getLink().'">'.$user->getFormatName().'</a>';
			}
