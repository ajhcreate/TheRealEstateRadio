...
class PlgJsnSocialconnect extends JPlugin
{
	public function __construct(& $subject, $config)
	{
		...
		if(!(JFactory::getApplication()->input->get('option',false)=='com_jsn' && JFactory::getApplication()->input->get('tmplsocial',false)))
		{
			...
			if (JSN_TYPE == 'pro') SocialConnect::init();
		}
		else
		{
		...
		if ( ! isset( $cache ) ) {
			...
			if ( $this->api->enabled && $this->api->application_id && $this->api->application_secret ) {
				...
				if ( ! $JS_loaded++ ) {
					...
					$script= "jQuery(document).ready(function(){jQuery('.socialconnect').append('<div><button class=\"facebook_button facebook zocial\">".$button_text."</button></div>');";
		...
		if ( ! isset( $cache ) ) {
			if ( $this->api->enabled && $this->api->consumer_key && $this->api->consumer_secret ) {
				...
				if ( ! $JS_loaded++ ) {
					...
					$script= "jQuery(document).ready(function(){jQuery('.socialconnect').append('<div><button class=\"twitter_button twitter zocial\">".$button_text."</button></div>');";
...
class SocialConnectGoogle extends SocialConnect {
			...
			if ( $this->api->enabled && $this->api->client_id && $this->api->client_secret && $this->api->api_key ) {
				...
				if ( ! $JS_loaded++ ) {
					...
					$urlParams		=	array(	'response_type=code',
												...
												'scope=' . urlencode( 'https://www.googleapis.com/auth/plus.me https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/userinfo.email' )
											);
					...
					$script= "jQuery(document).ready(function(){jQuery('.socialconnect').append('<div><button class=\"google_button googleplus zocial\">".$button_text."</button></div>');";
	...
	public function getConnectUser( $id = null, $user = null ) {
		...
		if ( ! isset( $cache[$id] ) ) {
			...
			$fields										=	array( 'id', 'nickname', 'displayName', 'image' );
			$mapping									=	explode( '|*|', $this->sync->fromfields );
			...
			$request									=	SocialConnect::httpRequest( 'https://www.googleapis.com/plus/v1/people/' . $id . '?fields=' . urlencode( implode( ',', $fields ) ) . $token . '&userIp=' . urlencode( array_pop( $ip_array ) ) );
			...
			if ( ( $request['http_code'] == 200 ) && ( ! $request['error'] ) ) {
				...
				if ( $error ) {
					...
					$cache[$id]							=	new stdClass();
					$cache[$id]->id						=	stripslashes( SocialConnect::getParam( $resultsArray, 'id', null ) );
					$cache[$id]->username				=	JApplication::stringURLSafe( SocialConnect::getParam( $resultsArray, 'nickname', null ) );

					if ( ! $cache[$id]->username ) {
						$cache[$id]->username			=	JApplication::stringURLSafe( SocialConnect::getParam( $resultsArray, 'displayName', null ) );
					}

					$cache[$id]->name					=	stripslashes( SocialConnect::getParam( $resultsArray, 'displayName', null ) );

					$image								=	SocialConnect::getParam( $resultsArray, 'image', null );

					$cache[$id]->avatar					=	( isset( $image->url ) ? preg_replace( '/\?sz=\d+/', '', stripslashes( SocialConnect::getParam( get_object_vars( $image ), 'url', null ) ) ) : null );
...
class SocialConnectLinkedIn extends SocialConnect {
	...
	public function getJS() {
		...
		if ( ! isset( $cache ) ) {
			...
			if ( $this->api->enabled && $this->api->api_key  && $this->api->secret_key ) {
				...
				if ( ! $JS_loaded++ ) {
					$perms			=	array( 'r_basicprofile', 'r_emailaddress' );
					...
					$script= "jQuery(document).ready(function(){jQuery('.socialconnect').append('<div><button class=\"linkedin_button linkedin zocial\">".$button_text."</button></div>');";
	...
	public function getConnectUser( $id = null, $user = null ) {
		...
		if ( ! isset( $cache[$id] ) ) {
			...
			$request							=	SocialConnect::httpRequest( 'https://api.linkedin.com/v1/people/' . ( $id == 'me' ? '~' : 'id=' . $id ) . ':(' . implode( ',', $fields ) . ')' . $token );

			if ( ( $request['http_code'] == 200 ) && ( ! $request['error'] ) ) {

				$resultsArray					=	(array) json_decode( json_encode( new SimpleXMLElement( $request['results'] ) ) );
		...
		if ( $this->api->enabled && $this->api->api_key  && $this->api->secret_key ) {
			...
			if ( ! $errorResponse ) {
				...
				if ( ( $request['http_code'] == 200 ) && ( ! $request['error'] ) ) {
					$resultsArray			=	(array) json_decode( $request['results'] );
					$accessToken			=	stripslashes( SocialConnect::getParam( $resultsArray, 'access_token', null ) );
					$request				=	SocialConnect::httpRequest( 'https://api.linkedin.com/v1/people/~:(id,formatted-name,email-address,picture-url,public-profile-url)?oauth2_access_token=' . $accessToken );
		...
		if ( ! isset( $cache ) ) {
			...
			if ( $this->api->enabled && $this->api->application_id && $this->api->application_secret ) {
				...
				if ( ! $JS_loaded++ ) {
					...
					$script= "jQuery(document).ready(function(){jQuery('.socialconnect').append('<div><button class=\"instagram_button instagram zocial\">".$button_text."</button></div>');";
