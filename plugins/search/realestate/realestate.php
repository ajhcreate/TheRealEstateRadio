...
require_once JPATH_SITE .  '/components/com_finder/helpers/route.php';
 ...
 */
class PlgSearchRealestate extends JPlugin
{
	/**
	 * Constructor
	 *
	...
	function onContentSearch( $text, $phrase='', $ordering='', $areas=null )
	{
		...
		//$query->select('a.name AS title, "" AS created, a.link AS text, ' . $case_when."," . $case_when1);
				...
				//$query->from('#__guru_task AS a');
				//$query->innerJoin('#__categories as c ON c.id = a.catid');
				//$query->where('('. $where .')' . 'AND a.published IN ('.implode(',', $state).') AND c.published = 1 AND c.access IN ('. $groups .')');
				
				//$query->from('#__guru_task AS a');
				//$query->where('('. $where .')' . 'AND a.published IN ('.implode(',', $state).')');
				//$query->order($order);
 		$query = 'select name AS title,"" AS created, "" AS text,"" AS section, "1" AS browsernav from #__guru_task where name like "%'.$text.'%"';
