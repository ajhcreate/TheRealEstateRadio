...
function accessToLesson($lesson_id, $course_id, $jump_module = ''){
    ...
    <?php
		...
		if($tmpl == "component"){
			$lesson_class = "gru-lesson-content modal-lesson-content";
			$is_modal = TRUE;
		}
	?>
                                                    ...
                                                    ?>
                                                            <form onsubmit="return validateQuizQuestions();" method="post" action="" name="quizz_exam" id="quizz_exam">
                                                                <?php
                                                                    ...
                                                                    if($result == 1){
                                                                        $sql = "SELECT show_countdown, max_score, nb_quiz_select_up, time_quiz_taken FROM #__guru_quiz WHERE id=".intval($id);
                                                                        $database->setQuery($sql);
                                                                        ...
                                                                        $sql = "SELECT score_quiz FROM #__guru_quiz_question_taken_v3 WHERE user_id=".intval($user_id)." and quiz_id=".intval($id)." and pid=".intval($step->pid)." ORDER BY id DESC LIMIT 0,1";
                                                                        $database->setQuery($sql);
                                                                        $result_q = $database->loadObject();
                                                                        
                                                                        $sql = "SELECT count(id) as time_quiz_taken_per_user FROM #__guru_quiz_question_taken_v3 WHERE user_id=".intval($user_id)." and quiz_id=".intval($id)." and pid=".intval($step->pid)." ORDER BY id DESC LIMIT 0,1";
                                                                        $database->setQuery($sql);
                                                                        ...
                                                                        if(@$result_q->score_quiz >= intval($result->max_score)){
																			$pass = 1;																																		
                                                                            ...
                                                                            if($callback == 0 || $callback == NULL){
																				if($result->show_countdown == 0){
                                                                                    if(trim(@$step->layout_media[0]) != ""){
                                                                                        ...
                                                                                        if(isset($id)){
                                                                                            $sql = "SELECT limit_time, limit_time_f, show_finish_alert from #__guru_quiz WHERE id=".intval($id);
                                                                                            $database->setQuery($sql);
                                                                                            $result = $database->loadObject();
                                                                                        }
                                                 ...
                                                 <div id="g_jump_button_ref_1">
                                                                ...
                                                                <?php
																		...
																		elseif(!accessToLesson($jump_cid, $course_id, $jump_details["0"]["type_selected"]) && $accessJumpB ==1){
																			?>
                                                                            <input class="btn btn-danger" type="button" name="JumpButton" value="<?php echo $step->layout_jump[0]->text; ?>" onclick="document.getElementById('id-lesson-<?php echo $jump_cid; ?>').click();" />
                                                                <?php
																		}
                                                                ...
                                                                <?php
                                                                        ...
                                                                        elseif(accessToLesson($jump_cid, $course_id, $jump_details["0"]["type_selected"]) && ($coursetype_details[0]["course_type"] == 0 || ($coursetype_details[0]["course_type"] == '1' && $coursetype_details[0]["lesson_release"] == '0'))){
                                                                        ?>
                                                                            <input class="btn btn-danger" type="button" name="JumpButton" value="<?php echo $step->layout_jump[1]->text; ?>" onclick="document.getElementById('id-lesson-<?php echo $jump_cid; ?>').click();" />
                                                                <?php
                                                                        }
                                                                        else{
                                                                            $lesson_jump_id = intval(JFactory::getApplication()->input->get("cid"));
                                                           ...
                                                           <div id="g_jump_button_ref_2">
                                                                ...
                                                                <?php
                                                                        ...
                                                                        elseif(accessToLesson($jump_cid, $course_id, $jump_details["0"]["type_selected"]) && ($coursetype_details[0]["course_type"] == 0 || ($coursetype_details[0]["course_type"] == '1' && $coursetype_details[0]["lesson_release"] == '0'))){
                                                                        ?>
                                                                            <input class="btn btn-danger" type="button" name="JumpButton" value="<?php echo $step->layout_jump[2]->text; ?>" onclick="document.getElementById('id-lesson-<?php echo $jump_cid; ?>').click();" />
                                                                <?php
                                                                        }
                                                                        else{
                                                                            $lesson_jump_id = intval(JFactory::getApplication()->input->get("cid"));
                                                                ...
                                                                <?php
                                                                        ...
                                                                        elseif(accessToLesson($jump_cid, $course_id, $jump_details["0"]["type_selected"]) && ($coursetype_details[0]["course_type"] == 0 || ($coursetype_details[0]["course_type"] == '1' && $coursetype_details[0]["lesson_release"] == '0'))){
                                                                        ?>
                                                                            <input class="btn btn-danger" type="button" name="JumpButton" value="<?php echo $step->layout_jump[3]->text; ?>" onclick="document.getElementById('id-lesson-<?php echo $jump_cid; ?>').click();" />
                                                                <?php
                                                                        }
                                                                        else{
                                                                            $lesson_jump_id = intval(JFactory::getApplication()->input->get("cid"));
