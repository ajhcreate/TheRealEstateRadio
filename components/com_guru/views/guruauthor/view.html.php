...
class guruViewguruauthor extends JViewLegacy {
	...
	function authoraddcourse($tpl =  null ){
		...
		foreach($plans as $plain ) {
			...
			$plains_html .= "<td style='padding:0.5em;' nowrap>".$positionB."<input type='text' class='uk-form-width-small gru-plan-price' id='subscription_price_".$k++."' name='subscription_price[" . $plain->id . "]' value='" . $price . "' />".$positionA."</td>";
			$plains_html .= "</tr>\n";
		}
		$plains_html .= "</table>";
		...
		foreach($renewals as $plain ) {
			...
			$plains_html2.= "<td style='padding:0.5em;' nowrap>".$positionB."<input type='text' id='renewal_price_".$k++."' class='uk-form-width-small gru-plan-price' name='renewal_price[" . $plain->id . "]' value='" . $price . "' />".$positionA."</td>";
			$plains_html2 .= "</tr>\n";
		}
		$plains_html2 .= "</table>";
	...
	function getCountEssaysForTeacher(){
		...
		$sql = "select q.id, q.question_content from #__guru_questions_v3 q, #__guru_quiz quiz where q.qid=quiz.id and q.type='essay' and quiz.author=".intval($user->id);
		$db->setQuery($sql);
		$db->query();
	...
	function addQuestion ($tpl =  null ) { 
		...
		$this->assignRef('medians', $medians);
	...
	function studentdetailslesson($tpl =  null){
		...
		$this->assignRef('all_lessons', $all_lessons);
