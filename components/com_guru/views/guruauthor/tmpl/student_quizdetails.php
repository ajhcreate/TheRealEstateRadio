        ...
        for($i=$k; $i<intval($pag * $per_page); $i++){      
            ...
            for($j=0; $j<count($media_associated_question); $j++){
                ...
                if(isset($media_that_needs_to_be_sent) && count($media_that_needs_to_be_sent) > 0){
                    $result_media[] = $helperclass->create_media_using_plugin($media_that_needs_to_be_sent["0"], $configs, '', '', '700px', 350);
                }   
            }
        ...
        <div class="guru-quiz__header">
            <ul>
                ...
                <li class="guru-quiz__header--alt2">
                    ...
                    <span><?php echo $user_name."'s"." ".JText::_("GURU_QUIZ_SCORE"); ?>: <?php echo intval($score)."%";?></span>
                </li>
                <?php
                ...
                <?php
                        ...
                        $link_quiz = JURI::root().'index.php?option=com_guru&view=gurutasks&catid='.$catid_req.'&module='.$module_req.'&cid='.$cid_req.'&tmpl=component&action_retake=retake';
