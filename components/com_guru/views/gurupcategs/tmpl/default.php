    ...
    if(isset($childrens) && is_array($childrens) && count($childrens) > 0){
        ...
        foreach($childrens as $key=>$value){
            ...
            if($result["0"] != "0"){
				...
				$categ_url = JRoute::_('index.php?option=com_guru&view=guruPcategs&task=view&cid='.$value["id"]."-".JFilterOutput::stringURLSafe(trim($value["name"])).'&Itemid='.intval($item_id));
        ...
        if ($layout == "1") {
            ...
            if ($img_align == 0) { // Centered image
                ...
                $return .= '<h3 class="uk-panel-title '.$style_categs->ctgs_categ_name.'">
                                <a href="'.JRoute::_('index.php?option=com_guru&view=guruPcategs&task=view&cid='.$course->id."-".$alias."&Itemid=".$item_id).'">'.$course->name.'</a>
                            </h3>';

                if($read_more == "0" && $edit_read_more == "0"){
                    $rmore = '<a style="float:'.$read_align.'" class="uk-button" href="'.JRoute::_('index.php?option=com_guru&view=guruPcategs&task=view&cid='.$course->id."-".$alias."&Itemid=".$item_id).'">'.JText::_("GURU_READ_MORE").'</a>';
                }
