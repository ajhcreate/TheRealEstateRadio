        ...
        <table class="uk-table uk-table-striped">
            <tr>
                ...
                <?php
                    ...
                    $sql .= " group by q.id ORDER BY qz.id DESC";
                    ...
                    foreach($my_quizzes as $key=>$value){
                        ...
                        if(count($quiz_taken_rows) > 0){
							...
							foreach($quiz_taken_rows as $key_row=>$value_row){
								...
								if(!$quiz_graded){
									$passfail = '<span style="color:#F89406;"> '.JText::_("GURU_QUIZ_PENDING").'</span>'; 
                                    $pcolor='color:#F89406;';
								}
                        ...
                        ?>
                                <tr>
                                    ...
                                    ?>
                                            <a href="<?php echo JRoute::_("index.php?option=com_guru&view=guruPrograms&task=view&cid=".intval($value["pid"])."-".@$alias."&Itemid=".intval($Itemid)); ?>">
                                                <?php echo $coursename; ?>
                                            </a>
                                    <?php
                                        }
                                    ...
                                    <td class="g_cell_5">
										<?php
											...
											if($retake_button === true){
												echo '<br /> <input type="button" class="uk-button uk-button-success uk-button-small" value="'.JText::_("GURU_RETAKE").'" onclick="window.location.href=\''.JRoute::_("index.php?option=com_guru&view=guruPrograms&task=view&cid=".intval($value["pid"])."-".@$alias."&Itemid=".intval($Itemid)."&quiz=".intval($id)).'\';" />';
											}
										?>
									</td>
                                    <td class="g_cell_6 g_hide_mobile"><?php echo $result_maxs."%"; ?></td>
                                    <td class="g_cell_7 ">
                                        <a class="g_hide_mobile" onclick="javascript:openMyModal(0, 0, '<?php echo JURI::root().'index.php?option=com_guru&view=guruauthor&task=student_quizdetails&layout=student_quizdetails&pid='.intval($value["pid"]).'&userid='.intval($user_id).'&quiz='.intval($value_row["quiz_id"]).'&id='.intval($value_row["id"]).'&tmpl=component'; ?>');" href="#">
                                            <?php echo $res."%".$passfail;?>
                                        </a>
                                    </td>
                                </tr>
                <?php
