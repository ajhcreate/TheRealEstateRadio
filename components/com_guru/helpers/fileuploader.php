...
class qqFileUploader {
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = FALSE){
        ...
        if ($this->file->save($uploadDirectory . $filename . '.' . $ext)){
			...
			$accepted_files = array("doc", "docx", "txt", "pdf", "csv", "htm", "html", "xhtml", "sxi", "xml", "sxw", "rtf", "odt", "css", "odp", "pps", "ppt", "ax", "amv", "asf", "avi", "mp4", "fla", "swf", "mov", "mpg", "mpeg", "rm", "aac", "aob", "ada", "au", "mid", "midi", "mp3", "ogg", "wav", "jpg", "jpeg", "png", "gif", "zip", "m4a");
...
switch ($mediaType){
	case "image":
		...
		$db->setQuery($sql);
		$res = $db->loadResult();
		$uploadPath=JPATH_BASE.DIRECTORY_SEPARATOR.$res.DIRECTORY_SEPARATOR.$_REQUEST['folder'].DIRECTORY_SEPARATOR;
		break;
	case "video":
		$sql = "SELECT videoin FROM #__guru_config LIMIT 1";
		$db->setQuery($sql);
		$res = $db->loadResult();
		$uploadPath=JPATH_BASE.DIRECTORY_SEPARATOR.$res.DIRECTORY_SEPARATOR;
		break;
	case "audio":
