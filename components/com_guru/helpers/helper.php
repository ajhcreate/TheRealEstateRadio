	...
	function createBreacrumbs(){
		...
		if($show_bradcrumbs == "1"){
			...
			if($controller == "guruPcategs" && trim($position) == ""){
				...
				$bradcrumbs .= '<ul id="g_breadcrumb" class="uk-breadcrumb">';
				$bradcrumbs .= 		'<li>';
				...
				while($parent_id != ""){
					if(intval($parent_id) != "0"){
						...
						$array_bradcrumbs[] = '<li><a class="pathway g_breadcrumb_link" href="'.JRoute::_("index.php?option=com_guru&view=guruPcategs&task=view&cid=".$parent_id."&Itemid=".$itemid).'">'.$parent_cat_name.'</a></li>';
					}
				...
				if($task == ""){
					...
					$bradcrumbs .= 		'<li><a class="pathway g_breadcrumb_link " href="'.JRoute::_("index.php?option=com_guru&view=guruPcategs&task=view&cid=".$catid."&Itemid=".@$itemid).'">'.$categ_name.'</a></li>';
				}	
				$bradcrumbs .= '</ul>';
			}
...
function createThumb($images, $folder, $new_size, $type, $folder_thumbs="thumbs"){
			...
			if($main_media->source=='url'){
				...
				if($main_media->source=='local'){	
					...
					elseif(strtolower($extension)=='mp4'){
						...
						$media = '
							<video controls '.$autoplay_html.' width="100%">
	...
	function createStudentMenu(){
		...
		$return = '
			<nav class="uk-navbar hidden-phone hidden-tablet">
	...
	function createPageTitleAndCart(){
		...
		$return = '
			<div class="uk-float-left pagetitle-cartbutton">
