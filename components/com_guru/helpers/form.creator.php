...
if (!class_exists("formCreator")) {
    ...
    class guruFormCreator {
        ...
        function guruFormCreator($form_name, $action, $form_id='') {
            $this->form_name = $form_name;
            $this->action = $action;
            $this->setSizeInputs( 30, 32 );
			$this->setImgPath(JURI::root().'components/com_guru/images/');
			if (empty($form_id)) {
...
var statescountry = new Array();\n";
        ...
        function toString() {
			...
			$this->imagePath =  str_replace ("http://", "https://", $this->imagePath);
...
//**************EXPIRE FIELD**************************************************************************************************
                    ...
                    case "expire":
						...
						$expDateMonth = $registry->get("expDateMonth", "");
						
						if(isset($expDateMonth) && $expDateMonth != ""){
							$mounth = trim($expDateMonth);
...
<tr{$style_row}>
	...
	<td>
	    ...
	    <select class='inputbox' id='{$params['year_name']}' name='{$params['year_name']}' onChange=\"javascript:checkExpire('{$params['month_name']}', '{$params['year_name']}')\">\n";
						...
						$session = JFactory::getSession();
						$registry = $session->get('registry');
						$expDateYear = $registry->get("expDateYear", "");
						
						if(isset($expDateYear) && $expDateYear != ""){
							$year_session = trim($expDateYear);
						}
                        for ($i = 0; $i < 20; $i++) {
...
<tr{$style_row}>
	<td>
        <input type='button' class='guru_continue_button' onclick='history.go(-1);' value='    Back    ' />&nbsp;
        <input type='submit' name='submitBtn' class='guru_checkout_button' value='{$params['value']}' size='30' maxsize='32'>
    </td>
	<td>&nbsp;
	...
	function checkExpire(month, year){
		...
		if( !CheckDateValue ){
			...
			if (true == $this->autoSubmit){
				$form .= '<img src="'.JURI::root().'plugins/gurupayment/paypaypal/paypal1.gif" border="0" name="submit" alt="PAYPAL">';
				$form .= "<br /><b>".JText::_("DSPAYMENT_WITH_PAYPAL")."</b>";
