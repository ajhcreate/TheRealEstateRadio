...
class guruControllerguruTasks extends guruController {
			...
			$count3 = $db->loadResult();
				if($count3 == 0){
					...
					if($count_firstpost == 0){
						$sql = "UPDATE #__kunena_topics set  posts=".($posts +1).", first_post_id=".$idmessage.",last_post_id=".$idmessage.", last_post_time='".strtotime($currentdate)."', last_post_userid=".$user->id.", last_post_message='".addslashes(trim($datamessage['message']))."', last_post_guest_name='".$user->username."' WHERE subject='".$name."'";
						$db->setQuery($sql);
						$db->query();
					}
					else{
						$sql = "UPDATE #__kunena_topics set  posts=".($posts +1).", last_post_id=".$idmessage.", last_post_time='".strtotime($currentdate)."', last_post_userid=".$user->id.", last_post_message='".addslashes(trim($datamessage['message']))."', last_post_guest_name='".$user->username."' WHERE subject='".$name."'";
						$db->setQuery($sql);
						$db->query();
					}
	...
	function getLessons() {
		...
		foreach ($modules as $module) {
			...
			foreach ($lessons as $key => $lesson) {
				$lessons[$key]['url'] = JURI::root() . "index.php?option=com_guru&view=gurutasks&catid=" . $program->catid . "&module=" . $module["id"] . "-" . $module["alias"] . "&cid=" . $lesson['id'] . "-" . $lesson["alias"] . "&tmpl=component&Itemid=" . $itemid;
	...
	function getCommentList( $lesson_id ) {
		...
		if($count > 0) {
			...
			$sql ="select count(id) from #__kunena_categories WHERE alias='" . $lesson_alias . "'";
			$db->setQuery($sql);
			$db->query();
			...
			if($enabled == 1){//if the plugin  is enabled
				if($board_less != 0){//if we have category created for the lesson
					$sql ="select numPosts from #__kunena_categories WHERE alias='" . addslashes(trim($lesson_alias)) . "' order by id desc limit 0,1";
					$db->setQuery($sql);
					$db->query();
					...
					if($user_id != 0 ){//if you are login
						...
						if($allow_stud == 0){
							...
							$sql ="select k.id, k.name, k.userid, u.email from #__kunena_messages k, #__users u WHERE k.subject='".addslashes(trim($lesson_name))."' and u.id=k.userid order by k.id desc";
							$db->setQuery($sql);
							$db->query();
							$resultid = $db->loadAssocList();
							...
							$sql ="select id from #__kunena_categories WHERE name='".addslashes(trim($lesson_name))."' order by id desc limit 0,1";
							$db->setQuery($sql);
							$db->query();
							$catkunena = $db->loadResult();

							$sql ="select id from #__kunena_topics WHERE subject='".addslashes(trim($lesson_name))."' order by id asc limit 0,1";
							$db->setQuery($sql);
							$db->query();
							$idmess = $db->loadResult();
	...
	function insertComment() {
		...
		$sql = "SELECT name FROM #__kunena_categories WHERE alias='".$aliaslesson."'";
		$db->setQuery($sql);
		$db->query($sql);
		...
		if(!isset($result2)){
			$sql = "INSERT INTO #__kunena_categories ( parent_id, name, alias, icon_id, locked, accesstype, access, pub_access, pub_recurse, admin_access, admin_recurse, ordering, published, channels, checked_out, checked_out_time, review, allow_anonymous, post_anonymous, hits, description, headerdesc, class_sfx, allow_polls, topic_ordering, numTopics, numPosts, last_topic_id, last_post_id, last_post_time, params) VALUES ( ".$resultidmodule.", '".addslashes($lesson_name)."', '".$aliaslesson."', 0, 0, 'joomla.group', 1, 1, 1, 8, 1, 2, 1, NULL, 0, '0000-00-00 00:00:00', 0, 0, 0, 0, '', '', '', 0, 'lastpost', 0, 0, 0, 0, 0, '{}')";
			$db->setQuery($sql);
			$db->query($sql);
			
			$sql = "SELECT id FROM #__kunena_categories WHERE alias='".$db->escape(trim($aliaslesson))."'";
			$db->setQuery($sql);
			$db->query($sql);
			$resultidlesson = $db->loadResult();
			
			if(trim($aliaslesson) != ""){
				$sql = "select count(*) from #__kunena_aliases where alias='".$db->escape(trim($aliaslesson))."' and type='catid' and item='".intval($resultidlesson)."'";
				$db->setQuery($sql);
				$db->query($sql);
				$count = $db->loadColumn();
				$count = @$count["0"];
				...
				if(intval($count) == 0){
					$sql = "INSERT INTO #__kunena_aliases (alias, type, item, state) VALUES ('".$db->escape(trim($aliaslesson))."', 'catid', '".intval($resultidlesson)."', 0)";
					$db->setQuery($sql);
					$db->query($sql);
				}
			}
		...
		// build html
		$sql ="select id, name, userid, email from #__kunena_messages WHERE subject='".$name."' order by id desc";
		$db->setQuery($sql);
		$db->query();
		$resultid = $db->loadAssocList();
