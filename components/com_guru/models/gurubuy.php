...
class guruModelguruBuy extends JModelLegacy {
	...
	function proccessSuccess($controller, $result, $stop = false){
		...
		if(isset($orderstatus) && trim($orderstatus) == "Pending"){
			...
			if(isset($courses) && trim($courses) != ""){
				...
				if(isset($all_courses) && is_array($all_courses) && count($all_courses) > 0){
					foreach($all_courses as $key => $value){
						...
						$modelProgram->emailForNewStudentEnrolled($course_id, $course_name, $authors, $first_name, $last_name);
						// send email to teacher for new student enroll ----------------------------------
			...
			$dispatcher->trigger('onCustomSuccessPayment', array(array("customer_id"=>$customer_id, "order_id"=>$order_id)));
			//-------------------
		}
		...
		$app->redirect(JRoute::_("index.php?option=com_guru&view=guruorders&layout=myorders&Itemid=".intval($item_id), false), JText::_("GURU_PAYMENT_SUCCESSFULLY"));
		return true;
	}
