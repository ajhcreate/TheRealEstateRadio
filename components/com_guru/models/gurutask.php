...
<embed width="'.$media->width.'" height="'.$media->height.'" name="plugin" src="'.JURI::root().$configs->videoin."/".$media->local.'" type="video/x-msvideo">
		...
		if(isset($media->type) && $media->type=='docs'){
			...
			if($media->source == 'local' && (substr($media->local,(strlen($media->local)-3),3) == 'txt' || substr($media->local,(strlen($media->local)-3),3) == 'xls' || substr($media->local,(strlen($media->local)-3),3) == 'pdf') && $media->width > 1 && $media->height > 0) {
				if($media->height == 0){
					$media->height = 600;
				}
				...
				elseif(substr($media->local,(strlen($media->local)-3),3) == 'xls'){
		...
		if(isset($media->type) && $media->type=='quiz' && @$media->published=='1' && strtotime($media->startpublish) <= strtotime($date) && ( strtotime($media->endpublish) >= strtotime($date) || $media->endpublish == "0000-00-00 00:00:00")){
			...
			$user_id = $user->id;
			$sql = "SELECT count(id) as time_quiz_taken_per_user FROM #__guru_quiz_question_taken_v3 WHERE user_id=".intval($user_id)." and quiz_id=".intval($media->id)." and pid=".intval($pid)." ORDER BY id DESC LIMIT 0,1";
			$database->setQuery($sql);
			$time_quiz_taken_per_user = $database->loadColumn();
			$time_quiz_taken_per_user = $time_quiz_taken_per_user["0"];
			
			$sql = "SELECT id as id_quiz_taken_per_user FROM #__guru_quiz_question_taken_v3 WHERE user_id=".intval($user_id)." and quiz_id=".intval($media->id)." and pid=".intval($pid)." ORDER BY id DESC LIMIT 0,1";
			$database->setQuery($sql);
			...
			if($time_quiz_taken_per_user["0"] > 0 && $task != "quizz_fe_submit"){
				...
				if($action_retake != "retake"){
					...
					$app->redirect(JURI::root().'index.php?option=com_guru&view=guruauthor&task=student_quizdetails&layout=student_quizdetails&pid='.intval($pid).'&userid='.intval($user_id).'&quiz='.intval($media->id).'&id='.intval($id_quiz_taken_per_user).'&tmpl=component&action=from_lesson&catid='.intval($catid)."&module=".intval($module)."&cid=".intval($cid));
				}
			}
		...
		if($id != 0 && $pid !=""){
			...
			if(count($intersect) == count($result2)){
				...
				if($completed != 1){
					...
					if(intval($course_certificate_term) == 1 || intval($course_certificate_term) == 2){
						// no certificate OR finish all lessons
						$set_course_completed = true;
					}
					elseif(intval($course_certificate_term) == 3 || intval($course_certificate_term) == 5){
	...
	function emailCertificate($pid){
		...
		if($email_sent == 0){
			...
			$query->insert('#__guru_logs');
			$query->columns(array($db->quoteName('userid'), $db->quoteName('emailname'), $db->quoteName('emailid'), $db->quoteName('to'), $db->quoteName('subject'), $db->quoteName('body'), $db->quoteName('buy_date'), $db->quoteName('send_date'), $db->quoteName('buy_type') ));
			$query->values(intval($id) . ',' . $db->quote('get-certificate') . ',' . '0' . ',' . $db->quote(trim($recipient)) . ',' . $db->quote(trim($subject_procesed)) . ',' . $db->quote(trim($body_procesed)) . ',' . $db->quote(trim(date("Y-m-d H:i:s"))) . ',' . $db->quote(trim(date("Y-m-d H:i:s"))) . ',' . "''" );
			$db->setQuery($query);
			$db->execute();
	...
	function InsertMyCertificateDetails($pid){
		...
		if(intval($count_cert) == 0){
			...
			foreach($resultauth as $key=>$value){
				...
				$sql = "insert into  #__guru_mycertificates (`course_id`, `author_id`, `user_id`, `emailcert`, `datecertificate`) values ('".intval($pid)."', '".intval($value)."', '".intval($id)."', '0', '".$current_date_cert."')";
				$db->setQuery($sql);
				$db->query();
			}
		}
	...
	function getAvgScoresQ($uid,$pid){
		...
		for($i=0; $i<count($quiz_id); $i++){
			if($id_final_exam !=0){
				$sql = "SELECT score_quiz FROM #__guru_quiz_question_taken_v3
	...
	function store_quiz_results(){
		...
		$sql = "INSERT INTO #__guru_quiz_question_taken_v3(user_id, question_ids, quiz_id, score_quiz, pid, date_taken_quiz, points, failed) VALUES ('".intval($user_id)."', '".$all_question_from_page."', '".intval($quiz_id)."', '".$score_quiz."', '".intval($pid)."', '".$date."', '".intval($summary_points)."', '".intval($autosubmit)."')";
		$db->setQuery($sql);
		$max_id = 0;
		if($db->query()){
