	...
	function store(){
		...
		if($data['guru_teacher'] == 2){
			...
			if(!$this->existAuthor($data['id'])){
				$sql = "INSERT INTO #__guru_authors (userid, gid, full_bio, images, emaillink, website, blog, facebook, twitter, show_email, show_website, show_blog, show_facebook, show_twitter, author_title, ordering, forum_kunena_generated,enabled, commission_id) VALUES('".intval($data['id'])."', 2, '".$db->escape($data["full_bio"])."','".$db->escape($data["images"])."', '".$db->escape($data["emaillink"])."', '".$db->escape($data["website"])."', '".$db->escape($data["blog"])."', '".$db->escape($data["facebook"])."', '".$db->escape($data["twitter"])."', '".$data["show_email"]."', '".$data["show_website"]."', '".$data["show_blog"]."', '".$data["show_facebook"]."', '".$data["show_twitter"]."',  '".$db->escape($data["author_title"])."', '".$data["ordering"]."', '', '".$data["enabled"]."', '".$id_commission."' )";
				$db->setQuery($sql);
				if(!$db->query()){
					$res = false;
				}
