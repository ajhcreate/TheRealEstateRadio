...
if ($status=='-1') 
	...
	function enroll(){
		...
		if(($result == 0) && $iscustomer == true){// add a new license		
			...
			$this->emailForNewStudentEnrolled($course_id, $course_name, $course_authors, $first_name, $last_name);
	...
	function emailForNewStudentEnrolled($course_id, $course_name, $course_authors, $first_name, $last_name){
		$db = JFactory::getDbo();
		$itemid = JFactory::getApplication()->input->get("Itemid", "0", "raw");
	...
	function getLessonDetails($program, $author, $lesson, $diff_date, $diff_start, $step_less, $start_date, $config, $lesson_details_for_quiz, $poz, $nr_lesson){
		$user = JFactory::getUser();
		$db = JFactory::getDbo();
