...
class guruModelguruauthor extends JModelList {
	...
	function getAuthorMyEssays(){
		...
		$sql = "select q.id, q.question_content from #__guru_questions_v3 q, #__guru_quiz quiz where q.qid=quiz.id ".$and_essays_filter." and q.type='essay' and quiz.author=".intval($user->id);
		$db->setQuery($sql);
		$db->query();
	...
	function getAuthorMyMediaCategories(){
		...
		$pagination->set("total", $this->_total);
		@$pagination->set("pagesTotal", ceil($this->_total / $pagination->get("limit")));
		@$pagination->set("pagesStop", ceil($this->_total / $pagination->get("limit")));
		@$pagination->set("pagesCurrent", ($limitstart / $limit + 1));
				...
				foreach ($thefiles as $files){
					if($files != ""){
						...
						if (intval($files)>0 && !in_array($files,$id_tmp_med_files_2_remove)) {
							...
							$query->columns(array($db->quoteName('id'), $db->quoteName('type'), $db->quoteName('type_id'), $db->quoteName('media_id'), $db->quoteName('mainmedia'), $db->quoteName('access'), $db->quoteName('order') ));
							$query->values("'', 'pmed'," . $db->quote(trim($progid)) . ',' . $db->quote(trim($files)) . ", '0', " . $db->quote(trim($data[$access])) . ", "  . $db->quote(trim($data[$order]))  );
							$db->setQuery($query);
							$db->execute();
						}
					}
...
<embed width="'.$vwidth.'" height="'.$vheight.'" type="video/x-msvideo" src="'.JURI::root().$configs->videoin."/".$the_media->local.'" name="plugin">
	...
	function storeMedia(){
		...
		if(intval($categ_id) == 0){
			...
			$name = $db->loadColumn();
			$name = @$name["0"];
			$sql = "insert into #__guru_media_categories (name, published, user_id) values ('".$name."\'s media', '1', '".intval($user_id)."')";
			$db->setQuery($sql);
			$db->query();
	...
	function storeLesson(){
		...
		if(intval($data['jumpbutton1'])!=0){
			$sql1="INSERT INTO #__guru_mediarel (type,type_id,media_id,mainmedia,text_no) VALUES ('jump','".$data['id']."','".intval($data['jumpbutton1'])."','0','0')";
			$db->setQuery($sql1);
			$db->query($sql1);
		}
		if(intval($data['jumpbutton2'])!=0){	
			$sql="INSERT INTO #__guru_mediarel (type,type_id,media_id,mainmedia,text_no) VALUES ('jump','".$data['id']."','".intval($data['jumpbutton2'])."','0','0')";
			$db->setQuery($sql);
			$db->query($sql);
		}
		
		if(intval($data['jumpbutton3'])!=0){
			$sql="INSERT INTO #__guru_mediarel (type,type_id,media_id,mainmedia,text_no) VALUES ('jump','".$data['id']."','".intval($data['jumpbutton3'])."','0','0')";
			$db->setQuery($sql);
			$db->query($sql);
		}

		if(intval($data['jumpbutton4'])!=0){
			$sql="INSERT INTO #__guru_mediarel (type,type_id,media_id,mainmedia,text_no) VALUES ('jump','".$data['id']."','".intval($data['jumpbutton4'])."','0','0')";
			$db->setQuery($sql);
			$db->query($sql);
		}
	...
	function getMedia(){
		...
		if($id==0){
			...
			$db->setQuery("SELECT count(*) FROM #__guru_questions_v3 WHERE qid='".$id."' ".$column_order);
			$db->query();
			$total = $db->loadColumn();
			$total = @$total["0"];
			$media->total = $total;
	...
	function addquestion ($qtext, $quizid, $question_type, $media_ids, $points, $true_false_ch, $question_id, $from_save_or_not, $ans_content) {
		...
		$sql = "INSERT INTO #__guru_questions_v3 (id, qid, type, question_content, media_ids, points, published, question_order) VALUES ('', '".$quizid."','".addslashes($question_type)."', '".addslashes($qtext)."' , '".$media_ids1."' , ".$points.", '1', ".$reorder.");";
		$db->setQuery($sql);
		if (!$db->query() ){
			$this->setError($db->getErrorMsg());
			return false;
		...
		if($question_type == 'true_false'){
			...
			$sql = "INSERT INTO #__guru_question_answers(id, answer_content_text, media_ids, correct_answer, question_id) VALUES ('', '".addslashes(JText::_("GURU_QUESTION_OPTION_TRUE"))."', '' , '".addslashes($true_default)."',".intval($id_question["0"]).");";
			$db->setQuery($sql);
			if (!$db->query() ){
				$this->setError($db->getErrorMsg());
				return false;
			...
			$sql = "INSERT INTO #__guru_question_answers(id, answer_content_text, media_ids, correct_answer, question_id) VALUES ('', '".addslashes(JText::_("GURU_QUESTION_OPTION_FALSE"))."', '' , '".addslashes($false_default)."', ".intval($id_question["0"]).");";
			$db->setQuery($sql);
			if (!$db->query() ){
				$this->setError($db->getErrorMsg());
				return false;
		...
		if(!$db->query()){
			...
			if($all_essays_has_answers){
				...
				if(isset($essays) && count($essays) > 0){
					...
					foreach($essays as $key=>$essay_id){
						$sql = "select q.id, m.user_id, q.question_content, m.grade, m.feedback_quiz_results from #__guru_questions_v3 q, #__guru_quiz_essay_mark m where q.id = m.question_id and m.question_id=".intval($essay_id)." and m.user_id=".intval($user_id)." GROUP BY q.id, m.user_id";
						$db->setQuery($sql);
						$db->query();
